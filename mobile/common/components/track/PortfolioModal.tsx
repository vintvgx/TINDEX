...
export function PortfolioModalContent({ visible, onClose }: PortfolioModalProps) {
  ...
  return (
    <>
      <View className="pt-4 pb-4 px-6 border-b border-gray-800 flex-row items-center justify-between">
      ...
      >
        {/* Add position form */}
        <View className="mb-6">
          <Text className="text-gray-400 text-sm font-medium mb-3">Add position</Text>
          <View className="bg-gray-800/60 rounded-xl border border-gray-700 p-4">
            <View className="mb-3">
              <Text className="text-gray-400 text-xs mb-1">Ticker</Text>
              <TextInput
                value={ticker}
            ...
            <View className="mb-3">
              <Text className="text-gray-400 text-xs mb-1">Shares</Text>
              <TextInput
                value={shares}
            ...
            <View className="mb-4">
              <Text className="text-gray-400 text-xs mb-1">Average cost ($)</Text>
              <TextInput
                value={averageCost}
            ...
            <TouchableOpacity
              ...
              className={`rounded-lg py-3 items-center ${
                canSave && !upsertMutation.isPending ? "bg-blue-600" : "bg-gray-700"
              }`}
            >
        ...
        {/* List */}
        <View>
          <Text className="text-gray-400 text-sm font-medium mb-3">Your positions</Text>
          {isLoading ? (
            <View className="py-8 items-center">
              <ActivityIndicator size="large" color="#10B981" />
              <Text className="text-gray-400 mt-2">Loading portfolio…</Text>
            </View>
          ) : error ? (
            <View className="py-6 rounded-xl bg-red-500/10 border border-red-500/30 px-4">
              ...
              <TouchableOpacity onPress={() => refetch()} className="mt-2">
                <Text className="text-blue-400 text-sm">Tap to retry</Text>
              </TouchableOpacity>
            </View>
          ) : positions.length === 0 ? (
            <View className="py-8 rounded-xl bg-gray-800/40 border border-gray-700">
            ...
            <View className="gap-2">
              {positions.map((pos) => (
                <View
                  key={pos.id}
                  className="flex-row items-center justify-between bg-gray-800/60 border border-gray-700 rounded-xl px-4 py-3"
                >
                  <View className="flex-1">
                    <Text className="text-white font-semibold">{pos.ticker}</Text>
