...
export default function TickerScreen() {
  ...
  if (error) {
    return (
      <View className="flex-1 bg-gray-50 justify-center items-center px-4">
        ...
        <Text className="mt-4 text-red-600 text-lg font-semibold">
  ...
  const renderTabButton = (tab: "Summary" | "Analytics" | "Financials") => (
    <Pressable
      ...
      <Text
        className={`text-lg font-medium ${activeTab === tab ? "text-black" : "text-gray-500"}`}>
        {tab}
      </Text>
    </Pressable>
  );
        ...
        <View className="flex-row justify-between mt-4">
          {periods.map((period) => (
            <Pressable
              ...
              <Text
                className={`text-sm font-medium ${selectedPeriod === period ? "text-black" : "text-gray-600"}`}>
                {period}
              </Text>
            </Pressable>
          ))}
        ...
        <View className="flex-row items-center mb-4">
          ...
          <View className="items-end">
            ...
            <View className="flex-row items-center">
              ...
              />
              <Text
                className={`ml-1 font-medium ${stockData.change >= 0 ? "text-green-600" : "text-red-600"}`}>
                {stockData.change >= 0 ? "+" : ""}
